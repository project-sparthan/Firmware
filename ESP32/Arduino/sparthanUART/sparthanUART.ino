...
void uartSendBuffer(uint8_t data[], uint16_t dataType) {

  static uint8_t txCount = 0;
  txCount++;
  memset(txBuffer, 0, sizeof(txBuffer));
  txBuffer[0] = START_OF_FRAME;
  memcpy(txBuffer + OFFSET_DATA_LENGTH, (uint16_t) sizeof(data), sizeof(uint16_t));
  txBuffer[3] = txCount;
  Append_CRC8_Check_Sum(txbuffer, 5);
  memcpy(txBuffer + OFFSET_DATA_TYPE, (uint16_t) dataType, sizeof(uint16_t);
  memcpy(txBuffer + OFFSET_
